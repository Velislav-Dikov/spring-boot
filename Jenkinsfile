    ...
    stages {
        ...
        stage('app_test_and_moving_reports') {
			...
			dir ('testing-junit5-mockito') {
				...
				sh 'echo '_____________________________Service status_________________________________' >> java-app-report.html'
				sh 'ssh veso@192.168.1.130 sudo systemctl status java-app >> java-app-report.html'
				sh 'echo '_______________________curltest from a remote host__________________________' >> java-app-report.html'
